...
import Database.data.VaccinesDAO;
...
public class Vaccines {
    ...
    public static void vaccineVault() {
        VaccinesDAO.createVaccinesTableIfNotExists();
        if (availableVaccine.isEmpty()) {
            availableVaccine.add(new Vaccines("Influenza", 1, 0.5, 365, "V001"));
            availableVaccine.add(new Vaccines("Chickenpox", 2, 0.5, 120, "V002"));
            availableVaccine.add(new Vaccines("Tetanus", 1, 0.5, 1825, "V003"));
            availableVaccine.add(new Vaccines("Polio", 3, 0.5, 150, "V004"));
            availableVaccine.add(new Vaccines("Hepatitis B", 3, 1.0, 90, "V005"));
        }

        for (Vaccines vaccine : availableVaccine) {
            VaccinesDAO.insertVaccine(vaccine.getVaccineId(), vaccine.getVaccineName(),
                    vaccine.getRequiredDoses(), vaccine.getDoseVolume(), vaccine.getIntervalDays());
        }
        System.out.println("Vaccines have been inserted into the database.");
    }
